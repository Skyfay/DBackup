...
- **ui**: The filters of the tables open like the other selects, take several values at once and keep their order while values are picked. Values without matches under the other filters wait at the end, and the type filter on the Connections page shows each adapter's icon.
...
- **docs**: The Storage Explorer guide describes the views by job and by destination, the timeline, the details of a backup and how current the lists are. The restore, verification, job, API and storage cache guides follow the new page.
