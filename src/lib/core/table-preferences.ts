...
export const VIEW_MODES = ["table", "cards", "split", "timeline"] as const;
export const ViewModeSchema = z.enum(VIEW_MODES);
export type ViewMode = z.infer<typeof ViewModeSchema>;

/** The views every list page has. Only the Storage Explorer adds the timeline. */
export type ListViewMode = Exclude<ViewMode, "timeline">;

/** A saved view for a page without a timeline, which shows the table instead. */
export function listView(view: ViewMode): ListViewMode {
    return view === "timeline" ? "table" : view;
}
