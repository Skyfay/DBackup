...
| Storage Explorer, the Backups tab | `src/app/dashboard/storage/storage-client.tsx`, `src/components/dashboard/storage/explorer/`. The Destinations tab waits for its canvas round, and the History and Alerts tabs of a destination and the restore page still have the old look. |
