...
- A list whose rows fall into groups, like the backups of a destination by job or the backups of an incremental job by chain, passes `view="split"` and draws the groups in `renderSplit` with the row's own cells through `flexRender`. A button in the toolbar turns the groups off, and only the flat table offers bulk actions. See `destination-backups.tsx` and `job-backups.tsx`.
