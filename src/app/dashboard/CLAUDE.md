...
- A field that picks a saved entry opens a list with a `pick` head that names where the entries come from. Each row says what the entry is and where it is in use, the entries that connections of the same kind use come first, and Edit sits on the row. Creating one is the New button beside the field, so the list only offers it when nothing matches. See `credential-picker-list.tsx`.
