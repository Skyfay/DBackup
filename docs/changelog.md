...
- **connections**: Creating a database or storage connection no longer does nothing when a required field sits in a part of the form that is not open. The form opens that part and shows what is missing.
- **connections**: The text fields of the connection forms are tied to their labels, so a click on a label puts the cursor in its field and screen readers name the field.
- **connections**: An empty required field of a new connection shows the adapter's own message, like "Bucket name is required", instead of Zod's "expected string, received undefined".
...
- **connections**: The form for adding and editing databases, backup destinations and directory sources lists its parts on the left, marks the ones that are done or still miss something, and shows one part at a time. The connection type and a storage connection's role are picked from cards that say what each one means, logins are picked or created in one field, and every switch says what it turns on.
...
- **docs**: The source, destination and first steps guides name the fields and parts of the new connection form, and the cloud drive guides describe signing in with an OAuth app. The storage adapter guide explains how the form places new fields.
