...
- A schedule is picked with `SchedulePicker`, in the job form and the preset dialog alike: a segmented switch for hourly, daily, weekly, monthly and cron, pills that take the tone of the form, times typed as 03:00, and a strip with the schedule in words, its time zone and its next runs. When runs would wait for a free slot it shows a small amber note by the rule the Overview uses (`findScheduleClash` in `lib/core/schedule-conflicts.ts`), with a time that has room, and nothing while the slots are enough.
